  ...
  return (
    <main className="moodboards-page">
      ...
      <div className="moodboards-header-bar">
        <Link href="/dashboard/moodboards/new" className="moodboards-new-btn">
          + New Moodboard
        </Link>
      </div>
          ...
          <div key={mb.id} className="moodboard-card">
            {mb.cover_image ? (
              <div className="moodboard-card-cover">
                <img src={mb.cover_image} alt={mb.title} />
              </div>
            ) : (
            ...
            <div className="moodboard-card-body">
              <div className="moodboard-card-meta">
                <span className={`moodboard-status moodboard-status--${mb.status}`}>
                  {mb.status}
                </span>
                {mb.projects?.title && (
                  <span className="moodboard-project">{mb.projects.title}</span>
                )}
              </div>
              <h3>{mb.title}</h3>
              <p>{mb.slides?.length || 0} slides</p>
            </div>
            <div className="moodboard-card-actions">
              <Link href={`/dashboard/moodboards/${mb.id}`} className="moodboard-action">
                Edit
              </Link>
              {mb.status === 'published' && (
                <a
